...
export const businessConfig = {
  ...
  tshirtPickupAddress: 'ул. Верхняя Первомайская, 47, корп. 11, подъезд 2, 3 этаж, кабинет 301',
  /**
  ...
  /**
   * Срок изготовления обычного (не срочного) заказа для сообщения клиенту.
   * День оформления НЕ считается — отсчёт со следующего дня. `days` — сколько
   * дней идёт работа: заказ готов на этот день, а клиенту называем диапазон
   * `days`…`days + 1` (например, +3…+4 = «11–12»), чтобы был запас на день.
   * Срочные заказы сюда не попадают — их срок подтверждает менеджер.
   */
  production: {
    days: 3,
  },
