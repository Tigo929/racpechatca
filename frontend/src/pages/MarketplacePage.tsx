...
import {
  AlertTriangle, Boxes, CheckCircle2, KeyRound, Package, Pencil, Plus,
  RefreshCw, ShoppingCart, Tags, Trash2,
} from 'lucide-react';
 ...
 *
 * Структура сознательно двухуровневая — сверху площадка, внутри этапы работы
 * (подключение → товары → цены и остатки → заказы). Так новый маркетплейс
 * добавляется вкладкой, а новый сценарий по Ozon — вкладкой второго уровня,
 * и страница не превращается в свалку кнопок, как кабинет самого Ozon.
 */
...
const PLATFORMS: {
  ...
  { key: 'OZON', label: 'Ozon', ready: true },
  { key: 'WB', label: 'Wildberries', ready: false },
  { key: 'YANDEX', label: 'Яндекс Маркет', ready: false },
];

type TabKey = 'connection' | 'products' | 'prices' | 'orders';
...
const TABS: {
  ...
  ready: boolean;
  /** Что здесь появится — показываем прямо в интерфейсе, чтобы был виден план. */
  soon?: string;
}[] = [
  ...
  { key: 'products', label: 'Товары', icon: Package, ready: true },
  {
    ...
    icon: Tags,
    ready: false,
    soon: 'Обновление цен и остатков по складам: правка списком, без перехода в кабинет Ozon.',
  },
  {
    key: 'orders',
    label: 'Заказы',
    icon: ShoppingCart,
    ready: false,
    soon: 'Заказы Ozon приходят в CRM как обычные заказы: с составом, сроком отгрузки и статусом.',
  },
];
...
function CheckStatus({ account }: { account: MarketplaceAccount }) {
  if (account.lastCheckOk === null) {
    return (
      <p className="text-xs text-gray-500">Связь ещё не проверялась.</p>
    );
  }
    ...
    <div className="flex items-start gap-2 rounded-lg bg-emerald-50 border border-emerald-100 p-3">
      ...
      <div className="text-xs text-emerald-800">
        ...
        {info?.warehouses?.length ? (
          <p className="mt-0.5">
            Склады: {info.warehouses.map((w) => w.name).join(', ')}
          </p>
...
function AccountForm({
  ...
  const save = useMutation({
    ...
    onSuccess: (account) => {
      ...
      if (account.lastCheckOk) toast.success('Кабинет подключён — связь есть');
        ...
        toast.error(
          account.lastCheckError ?? 'Кабинет сохранён, но связи нет',
          { duration: 6000 },
        );
      }
    },
  ...
  const canSave = title.trim().length >= 2 && externalId.trim().length >= 3 &&
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={editing ? 'Доступы кабинета' : 'Подключить кабинет Ozon'}
    >
...
function ConnectionTab({ marketplace }: { marketplace: EnumMarketplace }) {
...
/** Вкладка следующего этапа: честно говорим, что здесь будет. */
function SoonTab({ text }: { text: string }) {
  return (
    <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-8 text-center">
      ...
      <p className="mt-3 text-xs text-gray-400">
        Делаем по порядку: сначала подключение кабинета, затем товары.
      </p>
    </div>
  );
}
...
export default function MarketplacePage() {
  const [platform, setPlatform] = useState<EnumMarketplace>('OZON');
  ...
  return (
    <AppShell
      title="Маркетплейсы"
      subtitle="Работа с площадками по API — без кабинета Ozon"
      width="wide"
    >
      <div className="space-y-5">
        {/* Первый уровень — площадка. Неготовые площадки видны, но выключены:
            так понятно, что раздел рассчитан не только на Ozon. */}
        <div className="flex flex-wrap gap-2">
          {PLATFORMS.map((p) => (
            <button
              key={p.key}
              onClick={() => p.ready && setPlatform(p.key)}
              disabled={!p.ready}
            ...
            >
              {p.label}
              {!p.ready && <span className="ml-1.5 text-[11px]">скоро</span>}
            </button>
          ))}
        </div>

        {/* Второй уровень — этап работы с площадкой. */}
