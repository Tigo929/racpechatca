...
# 32. Архитектурное решение (FIX_01, 11.09.2026)

Принято рецензентом, реализовано в `web-photo` `feature/analytics-event-model` (cb2dd96).

```text
yclid TTL = 21 days
  метка Директа хранится с моментом получения и действует ровно 21 день —
  окно, в котором Метрика связывает офлайн-конверсии и заказы из CRM
  с визитом; новый клик заменяет старый и начинает новый отсчёт;
  прежние записи без даты недействительны и удаляются.

campaign UTM = last-touch within current session
  маркетинговая атрибуция заявки в CRM — последний набор utm_* внутри
  текущей вкладки (sessionStorage); новый набор заменяет предыдущий.

firstTouchUrl = entry-page dimension, not marketing attribution
  первая страница текущей вкладки — измерение поведения для анализа
  входных страниц и UX; рекламный источник заказа НЕ определяет.

conversionPageUrl = conversion-page dimension
  страница, на которой отправлена заявка.

yandexClientId — главный ключ связи заказа с посетителем; TTL yclid его не касается.
```

Для этапа 08 (`08_ANALYTICS_METRICS`): канал/источник — по данным визита
Яндекс Метрики, где возможно; UTM заявки трактовать как `last-touch UTM
in current session`; `firstTouchUrl` — только как входная страница; не называть
его first-touch marketing attribution.
