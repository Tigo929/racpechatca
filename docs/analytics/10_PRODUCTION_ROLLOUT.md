...
Отчёт — § 21. Остановлено на owner-smoke gate (§ 15): фиксирует владелец. DONE ставит Reviewer.
```
...
```text
§ 15 — ожидает владельца: https://raspechatkaa.ru/crm/analytics → «Поведение» (и на хосте sslip.io).
Цифры ниже — на момент сверки 00:28 15.09; к моменту проверки окно «7 дней» сдвинется и значения изменятся
(например, к 10:23 снимок целей за 7 дней уже даёт form_started 4 посетителя, lead_submitted 2):
  [ ] вкладка открывается; статус данных (Актуально / предупреждения о частичном периоде)
  [ ] общая воронка: 116 визитов → 13 начали (25 событий, 3 посетителя) → 4 отправили → 4 заявки (7 дней, 09–15.09)
  [ ] направления: фото / футболки / холсты / контакты; «шаг не измеряется» у каталога, кроя/цвета, попытки, загрузки
  [ ] ошибки форм (1 визит, поле «Контакт»), устройства (телефон 51 визит / 0 форм), страницы входа, пути
  [ ] «Требует внимания»: 2 карточки (Критично — устройства; Внимание — отвал фото), Факт / Гипотеза / Что проверить
  [ ] today / 7d / 30d; телефон — без горизонтального скролла; разделы этапа 09 (Обзор, Источники, …) работают
Фиксирует только владелец.
```
...
```text
1. Owner smoke § 15 → verdict Reviewer (10_BEHAVIOR_AND_FUNNELS / 10_PRODUCTION_ROLLOUT → DONE).
