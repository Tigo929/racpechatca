...
проверены на боевых данных — § 18. Owner smoke § 11 пройден владельцем 14.09.2026 (§ 18.8).
Все пункты Decision Gate (§ 17) закрыты со стороны исполнителя; DONE ставит Reviewer.
```
...
```text
DONE_PENDING_REVIEW — все проверки § 3–13 выполнены, owner smoke § 11 пройден (14.09.2026)
```
...
Owner smoke § 11 подтверждён владельцем 14.09.2026 после 20:20 MSK: «OWNER SMOKE §11 ПРОЙДЕН».
...
§ 11 (флаг true):  ПРОЙДЕН — подтверждение владельца 14.09.2026 (после 20:20 MSK), дословно:
  «OWNER SMOKE §11 ПРОЙДЕН. /crm/analytics под ADMIN открывается корректно. Dashboard включён, данные
  отображаются, пресеты Сегодня / 7 дней / 30 дней работают, Site funnel и CRM funnel разделены,
  предупреждение о неполном сопоставлении отображается корректно, разделы аналитики открываются,
  критических визуальных ошибок не обнаружено.»
  Чек-лист владельца:
  [x] /crm/analytics открывается под ADMIN, dashboard включён, данные отображаются
  [x] пресеты Сегодня / 7 дней / 30 дней работают (ориентиры 19:18 MSK: визиты 12 / 144 / 588,
      посетители 9 / 93 / 326, принятые 5 / 33 / 127, оплаты 7 / 10 / 70, выручка 16 975 / 66 455 / 229 231 ₽,
      прибыль 8 819 / 44 015 / 132 573 ₽ — живые данные, после новых заказов сдвигаются)
  [x] воронка сайта и воронка CRM — отдельные карточки
  [x] предупреждение о неполном сопоставлении (ClientID) отображается корректно
  [x] разделы Источники / Товары / Страницы / Качество данных открываются
  [x] критических визуальных ошибок нет
  [ ] телефон — владелец отдельно не отмечал (responsive подтверждён на копии production, § 64 этапа 09)
  скриншоты production владелец не прикладывал (по § 11 допустимо текстовое подтверждение)
```
...
4. § 11 owner smoke подтверждён владельцем текстом без скриншотов production (допустимо по § 11); пункт
   «телефон» отдельно не отмечен — responsive проверен на копии production (этап 09 § 64, mobile 390 px).
```
