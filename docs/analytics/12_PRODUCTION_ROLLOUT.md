...
```text
DONE_PENDING_OWNER_SMOKE (Reviewer, 17.09.2026: «12_PRODUCTION_ROLLOUT = DONE_PENDING_OWNER_SMOKE», 12_FIX_01 = DONE,
12_AUTOMATED_INSIGHTS = REVIEW; production master 1f8b7b4, Insights ENABLED; NEW FACTS 4 и 6 — отдельный infrastructure/CI debt,
не основание для FIX; первая v2 — наблюдение, не gate). Ожидается owner smoke § 27 (§ 36).
Ранее DONE_PENDING_REVIEW (исполнитель, 17.09.2026). Technical rollout выполнен 17.09.2026 09:10–10:37 MSK по команде Reviewer
...
5. Блок «Правил без вывода в последнем запуске» днём показывает диагностику часового запуска (3 записи), полная разбивка daily (≈50)
   видна только в первый час после 00:33 — по контракту, но неочевидно для владельца; вариант — показывать последний полный (daily)
   запуск или оба; решение Reviewer, код на production не менять (§ 36).
```
...
# 36. OWNER SMOKE — подготовка (17.09.2026, 12:18 MSK)

Reviewer поставил `12_PRODUCTION_ROLLOUT = DONE_PENDING_OWNER_SMOKE`; чек-лист владельца — 7 пунктов. Исполнитель в production-UI
под учётной записью владельца не входит; ниже — что владелец увидит, сверено с production-JSON (захват через домен 12:18 MSK →
сборка кандидата, `screenshots/12_insights/prod-*.png`). Результат фиксирует владелец («OWNER SMOKE STAGE 12 ПРОЙДЕН» / замечания).

| # | Пункт владельца | Что на production сейчас |
|---|---|---|
| 1 | вкладка открывается, текущие карточки | `raspechatkaa.ru/crm/analytics → Инсайты`: 8 карточек (1 «Внимание» + 7 «К сведению»), дисклеймер и границы данных в шапке, ошибок 0 |
| 2 | мобильные — сигнал для проверки, не причина | «На телефонах конверсия заметно ниже, чем на компьютерах» (Внимание, правило этапа 10): ФАКТ — доли 4,3 % / 9 %; блок «Гипотеза — не факт» с текстом «…данные это не различают. Причинность не установлена»; ЧТО ПРОВЕРИТЬ — пройти путь на телефоне (CHECK_MANUALLY) |
| 3 | подпись у карточек качества данных | три карточки (покрытие ClientID 15 %, 6 оплат без даты, «Фотопечать: шаг не измеряется») — блок подписан «Качество измерения — не поведение клиентов», чип «качество данных, не поведение» |
| 4 | фото: шаг «не измеряется», не 0 | карточка фотопечати: «…не измеряется шаг — «Просмотр каталога / товара»… Значение таких шагов — not_measured, не 0»; во вкладке «Поведение» этот шаг подписан «не измеряется», числа нет |
| 5 | изменение 12.09 — «Окна несопоставимы» | «Оценка изменения «Деплой сайта 12.09…» обновилась: v10 — окна несопоставимы»; в ЧТО ПРОВЕРИТЬ — рекомендация этапа 11 дословно; процента роста в заголовке нет |
| 6 | у обычных карточек гипотеза отделена | «Гипотеза — не факт» (пунктирная рамка, курсив) у пяти карточек: мобильные, визиты 229 → 116, сдвиг источников, PHOTO 38 → 19, оценка 12.09 |
| 7 | блок «Правил без вывода» | внизу вкладки «Правил без вывода в последнем запуске: N» с разбивкой по причинам и текстом «Молчание правила — не подтверждение отсутствия проблем». **N зависит от последнего запуска**: после daily 09:35 было 49/50; днём последний запуск — часовой (2 детектора) → блок показывает **3** (оценка 12.09 уже в ленте, изменение-инцидент — недостаточно данных, данные свежие). Полная разбивка (50) видна в первый час после daily нового дня (00:33–01:33). Это поведение по контракту («в последнем запуске»), но для владельца неочевидно — OPEN DECISION 5 |

Дополнительно: 390 px — без горизонтальной прокрутки; «Принять к сведению» доступна на каждой карточке (действие владельца по желанию).
