...
---
# 5h. Дашборд руководителя — этап 09 (14.09.2026; в production НЕ выложен)

| Факт | Подробности |
|---|---|
| Статус | Reviewer 14.09.2026: `00–08 = DONE`, `09 = READY_FOR_PRODUCTION_ROLLOUT`; план выкладки — `09_PRODUCTION_ROLLOUT.md` (PREPARED, ждёт «СТАРТ»): без миграций, scheduler не выключать, строка флага в серверный compose, включение отдельным шагом с backup `.env`, сверка из контейнера временным токеном в памяти процесса, owner smoke |
