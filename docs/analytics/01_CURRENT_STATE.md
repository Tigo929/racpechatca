...
| Факт | Подробности |
|---|---|
| Статус | Stage 10 = REVIEW: technical rollout выполнен 15.09 00:13–00:42 MSK по команде «СТАРТ» строго по `10_PRODUCTION_ROLLOUT.md` — отчёт § 21; owner smoke § 15 пройден владельцем 15.09 (~10:34 MSK, «OWNER SMOKE STAGE 10 ПРОЙДЕН», § 21.16); со стороны исполнителя всё закрыто, DONE ставит Reviewer. FIX_01 (правило 11.1: переход оценивается только при совпадающих окнах измерения шагов, иначе `skipped[]` с кодом `PARTIAL_BEHAVIOR_PERIOD`; `FunnelStep.measuredFrom/transition`; параметр-шаги `availableFrom = null`) — реализован 15.09 в feature f2db719 (+ be7a282 вложенные таблицы панели), CRM 960 / панель 30, BEFORE/AFTER на боевых данных: 30д без ложных «118 → 2» / «20 → 0», DEVICE_GAP без изменений; выложен в production 15.09 11:20–11:26 MSK по команде «СТАРТ» (master b04681f; § 22 план / § 23 отчёт rollout-документа): BEFORE = AFTER по 54 шагам воронок, FUNNEL_DROPOFF photo/canvas/tshirt на 7д/30д → skipped PARTIAL_BEHAVIOR_PERIOD, DEVICE_GAP тот же, Stage 06/09 без регресса, тики SUCCESS; DONE ставит Reviewer |
| Production | `master` = b04681f (FIX_01, ff 15.09 11:20:48; до этого 80921d9 с 00:18:31); auto-update FIX_01: backend `34a82271…` 11:24:57–11:25:29 («No pending migrations»), frontend `23e12a79…` 11:25:35–11:26:08; Stage 10 base: backend `d8c9dd73…` 00:21:33–00:22:06 (миграция `20260914200000` применена на старте, 78 applied), frontend `844bb9f8…` 00:22:11–00:22:44; backups `.bak-stage10-20260915-0013` (compose, .env) и `premigration_stage10_behavior_20260915_001341.sql.gz`; расписание аналитики выключалось 00:17–00:31 на время миграции и initial sync; маршрут `/analytics` на домене raspechatkaa.ru добавлен во второй белый список nginx 15.09 00:01 (NEEDS_FIX; фикс в репо — `frontend/nginx.conf` комментарий, web-photo `deploy/nginx-domain.conf`) |
...
---
# 5j. Рост и изменения — этап 11 (15.09.2026; в production с 15.09.2026 23:17 MSK)
...
| Rollout | Выполнен 15.09 23:10–23:23 MSK по `11_PRODUCTION_ROLLOUT.md` (СТАРТ Reviewer): master 3ac9be8 → 5922175 (код 0c3b84a), миграция 20260915130000 на старте контейнера (59 таблиц), раздел выложен выключенным (compose + `ANALYTICS_GROWTH_ENABLED` default false), OFF-gate 8/8 (§ 9.5 — 400 от ValidationPipe до флага, записей 0 — отклонение на решение Reviewer), enable 23:17:35 (.env `ANALYTICS_GROWTH_ENABLED=true`, backend recreate --no-deps); в реестре A «деплой 12.09» (siteLeadRate → INCOMPARABLE, 3,92 % → 7,41 % не вывод) и B «инцидент 14–15.09» (COMPLETED, confounder); сверка A = B = C diff 0; версии неизменяемы; лок первичной метрики; P&L = /reports/weekly diff 0; PII 0; cold evaluate 2,23 с; 13 автоциклов 16.09 SUCCESS (v9 scheduler 00:17); Stage 06/09/10 diff 0; owner smoke § 27 ПРОЙДЕН владельцем 16.09 (~14:42 MSK); отчёт § 35; DONE ставит Reviewer |
...
---
# 5k. Инсайты — этап 12 (16.09.2026; в production с 17.09.2026 09:33 MSK)

| Факт | Подробности |
|---|---|
| Статус | Stage 12 = REVIEW (Reviewer 17.09: `12_FIX_01 = DONE`, `12_PRODUCTION_ROLLOUT = DONE_PENDING_OWNER_SMOKE`; DONE — после owner smoke § 27, чек-лист владельца — `12_PRODUCTION_ROLLOUT.md` § 36): technical rollout выполнен 17.09 09:10–10:37 MSK по команде «СТАРТ» строго по `12_PRODUCTION_ROLLOUT.md` (отчёт § 35): master 1f8b7b4, backend d8590e7c9e67 / frontend 1485aa9b3150, миграция 20260916120000 применена 09:29:31 (83 миграции, 62 таблицы), OFF-gate 9/9 (404 до ValidationPipe), `ANALYTICS_INSIGHTS_ENABLED=true` с 09:33:16, первый запуск 09:35 (daily, окна 03–09.09 → 10–16.09): 19 детекторов, 8 карточек (ATTENTION DEVICE_GAP этапа 10; INFO — оценка 12.09 INCOMPARABLE, визиты 229 → 116, сдвиг источников 75 → 59 %, PHOTO 38 → 19, ClientID 15 %, 6 оплат без даты, «Фотопечать: шаг не измеряется»), 49 причин молчания, ошибок 0; ручной run идемпотентен; A = B = C diff 0; not_measured нигде не 0; causal claims 0; PII 0; perf: daily 4,3 с, hourly 35 мс, GET ≤ 0,5 с; тики 09:35 daily / 10:33 hourly SUCCESS; Stage 06/09/10/11 без регресса; owner smoke § 27 и DONE — владелец и Reviewer. До этого: Reviewer 16.09 — implementation + FIX_01 приняты (кандидат 1f8b7b4); rollout-план был READY_FOR_REVIEW (34 раздела: git/pre-deploy gate, backup, review миграции, deploy OFF, OFF-gate с 404 до ValidationPipe, решение о моменте включения — сразу после OFF-gate, controlled enable / first run, сверка A = B = C, все 19 детекторов, DEVICE_GAP / EVENT_NOT_MEASURED / INCOMPARABLE / quality, язык, lifecycle, диагностика, stale, гейты, планировщик, Метрика 0, PII, perf, регресс 06/09/10/11, UI, owner smoke, STOP, rollback); production не менялся, старт — по команде «СТАРТ». FIX_01 выполнен 16.09 (5f90e68 / 0d7e901 + docs): реализовано в `feature/analytics-foundation` (b983676 / 0a0c6cf / 8753faf + docs), проверено на копиях `crm_stage12_test` (16:07 и 19:41 MSK, удалены); production не тронут; отчёты `12_AUTOMATED_INSIGHTS.md` § 49 и § 50 (FIX_01) |
...
| Сверка на копии | окна 02–08.09 → 09–15.09: 4 карточки ([CRITICAL] DEVICE_GAP этапа 10, [INFO] оценка 12.09 INCOMPARABLE, [INFO] покрытие ClientID 6 %, [INFO] визиты 244 → 126), 51 причина молчания (LOW_SAMPLE 30, MEASUREMENT_DEFINITION_CHANGED 3, IMMATURE 4, …); A = B = C diff 0; повторный запуск без версий; feed 0,2–0,3 с через туннель. После FIX_01 (копия 19:41): 19 детекторов, 6 карточек (+ [INFO] «Воронка «Фотопечать»: шаг не измеряется — анализ отвала ограничен»; + [INFO] 11 оплат без даты — из свежих данных), 52 причины молчания (tshirt — LOW_SAMPLE 13 визитов на входе; canvas — шаг на сайте не существует); A = B (5, diff 0) = C (diff 0); повтор → unchanged 5 |
| Тесты | CRM 1078 (99 suites; этап 12 — 63, из них FIX_01 — 9), панель 42 (insights 7); build OK; lint чист по аналитике |
...
| Факт | Подробности |
|---|---|
| Статус | Stage 13 = READY_FOR_PRODUCTION_ROLLOUT (Reviewer APPROVED 17.09); rollout-план `13_PRODUCTION_ROLLOUT.md` = READY_FOR_REVIEW (gate-ы A CRM code / B server hardening / C web-photo CI / D validation / E backup / F retention dry-run / G secrets / H debts — каждый с PRECHECK/ACTION/VERIFY/STOP/ROLLBACK; production не менялся, старт — по «СТАРТ»): `feature/analytics-foundation` 806e328 + 51f27ae + docs; `web-photo` `feature/ci-safety` 441d795 (production-ветка сайта не менялась); отчёт `13_RELIABILITY_SECURITY.md` § 32; production master 1f8b7b4 не менялся |
