...
| Три пространства | site (Метрика: визиты, lead_submitted, CRM-цели по сопоставленным заказам), crm (все заказы: leadAt / acceptedAt / paidAt / cancelledAt / realizedAt из `deriveOrderLifecycle`), pnl (`ReportsService.pnlForRange` — та же математика, что `/reports/monthly`) |
