...
> **Статус после финальной приёмки (этап 14, 22.09.2026): NOT OBSERVED /
> NOT APPLICABLE FOR CURRENT DATA.** Колонки `utmSource…utmTerm` в CRM есть и
> заполняются кодом, но за окна приёмки (30 дней 23.08–21.09 и 7 дней
> 15.09–21.09) UTM-меток не было ни у одного из 144 заказов, и в самой Метрике
> помеченного трафика почти нет (588 визитов без меток, 11 с `chatgpt.com/`).
> Проверить UTM-цепочку end-to-end на реальных данных было **не на чем**, поэтому
> пункт не считается доказанным: он ждёт первой кампании с метками. Реклама в этом
> окне опознавалась по `yclid` и по классификации источников Метрики.
...
(кроме секунд «возраста»). Тесты: CRM 912, панель 21 (vitest). Production rollout выполнен 14.09.2026
18:44–20:17 MSK по команде «СТАРТ» строго по `09_PRODUCTION_ROLLOUT.md`: master = be591d3 (ff), образы backend
4d8cdf1c / frontend 2b96a48c, миграций нет (77 up to date), выключенное состояние проверено (401/404, карточка
«Раздел выключен» подтверждена владельцем), флаг включён 19:16:39 с backup .env, API 401/403/400/200, сверка
...
```text
REVIEW (14.09.2026) — поведенческий слой поверх локальных агрегатов Метрики: аудит production-кода сайта
(14 событий с целями, 10 без целей, 9 мёртвых; DATA GAPS G1–G8; Logs API не включён), пять новых наборов
Reports API (цели по устройствам и страницам входа, параметры визитов из белого списка, пути-агрегаты,
вовлечённость) + снимок посетителей по целям; воронки в трёх подписанных единицах (события / целевые визиты /
...
11_PRODUCTION_ROLLOUT.md § 35; owner smoke § 27 пройден владельцем 16.09 (§ 35.18). DONE (Reviewer, 16.09.2026):
11_GROWTH_AND_EXPERIMENTS = DONE, 11_PRODUCTION_ROLLOUT = DONE, production master 5922175. Следующий этап —
12_AUTOMATED_INSIGHTS.md (READY_FOR_IMPLEMENTATION, только feature-ветка).
```
...
```text
ACCEPTED / PASS (Reviewer, 22.09.2026) — docs/analytics/14_FINAL_ACCEPTANCE.md.
Приёмка проведена ретроспективно и только на чтение: записей в production 0.
Цепочка трафик → заявка → заказ → статусы → Метрика → деньги → dashboard → вывод
проверена на реальных данных, внутренние сверки (traffic, crm, pnl) дали 0 расхождений.
Baseline: CRM master fe88cfe, сайт 441d795. Фундамент сквозной аналитики считается
готовым к эксплуатации; дальнейшие задачи — качество и полнота данных
(HARDENING_BACKLOG.md § 5), а не архитектура аналитики.
```
...
## 15_AI_ANALYTICS_REPORT

Статус:

```text
ACCEPTED / PRODUCTION (23.09.2026) — docs/analytics/15_AI_ANALYTICS_REPORT.md.
Production SHA 9ccd29351a728c7e43f58c1ad7b7bba894e6e6af; генерация только на чтение.
Один самодостаточный markdown (+ печатный HTML) со сводкой, сигналами, трафиком,
воронками, продажами, P&L, атрибуцией, качеством данных, трендами, аномалиями,
ростом, входом для прогноза, словарём метрик и инструкцией для внешнего ИИ.
Команда: npm run analytics:report -- --period=7d
```

Цель:

отдать всю принятую аналитику одним файлом внешнему ИИ и получить разбор бизнеса,
не пересказывая ему документацию этапов 00–14.
...
сделать получение аналитического отчёта пользовательским действием, а не командой в
терминале: кнопка в панели → выбор периода → «Сформировать отчёт» → скачать MD / HTML / PDF.

Предполагаемый объём (по описанию Reviewer, уточняется в ТЗ):

```text
- маршрут генерации отчёта в API поверх существующего генератора этапа 15 (своей
  бизнес-логики не добавляем — тот же report-model);
- экран в разделе аналитики: период, кнопка, состояние генерации, ссылка на скачивание;
- хранение последних отчётов и дата последней генерации;
- по желанию — автоматическая недельная/месячная генерация;
- PDF: либо печать HTML в браузере пользователя, либо отдельное решение на сервере.
```

Решения владельца (ТЗ 23.09.2026): доступ только ADMIN; файлы на диске сервера,
хранение 180 дней и не больше 50 готовых; облачный бэкап их не копирует; автоматического
расписания нет; PDF-движок в образ не добавляем — остаётся печать HTML из браузера.
...
одно определение происхождения заказа на всю систему: заявку создал сайт (`WEBSITE`)
или заказ завели в CRM вручную (`AVITO` по умолчанию, а также `OZON`, `WB`, `LOCAL`);
историческое «не доказано» — `UNKNOWN`. Происхождение заказа отделено от маркетинговой
атрибуции сайта: конверсия сайта считается только по населённости `WEBSITE`, делить
все заказы CRM на визиты сайта запрещено.

Объём:

```text
- additive-миграция перечисления (WEBSITE, UNKNOWN), новое поле не заводится;
- сервер ставит WEBSITE заявкам сайта; ручной заказ — выбранный источник либо AVITO;
- поле «Источник заказа» в создании и правке заказа, источник виден в карточке;
- разрез по происхождению в каноническом срезе каналов, включая деньги из P&L
  (ReportsService.pnlByOrigin — тот же buildPnl, второй формулы прибыли нет);
- блок «Каналы заказов» в панели: UNKNOWN виден, рекламные источники — отдельно;
- раздел ORDER ORIGIN в отчёте для ИИ: таблица, полнота классификации, методика;
- CLI исторической классификации: dry-run по умолчанию, --apply со снимком,
  повторный apply — 0 изменений.
```
...
10_BEHAVIOR_AND_FUNNELS — REVIEW (14.09.2026): аудит событий сайта против production-кода и целей счётчика,
