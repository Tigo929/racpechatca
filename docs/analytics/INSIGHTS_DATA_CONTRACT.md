...
default false). FIX_01 (16.09.2026): детектор quality.eventNotMeasured (19 детекторов), каноническая нагрузка без окон,
при неизменной нагрузке двигаются столбцы окна карточки; схема и миграция не менялись.
```
...
- Каноническая нагрузка для сравнения: JSON с отсортированными ключами, числа округлены до 1e-6, без `quality.freshness`,
  `generatedAt`, ссылок `sync_run` и (FIX_01) без `fact.period` / `fact.baselinePeriod` — окна сдвигаются каждый день и
  сами по себе не «новые данные»; факт меняется через свои числа и состав. Тот же хэш → без версии: обновляются
  `lastDetectedAt` и столбцы окна карточки (`periodStart/End`, `baselineStart/End` — карточка показывает текущее окно);
  другой хэш → `version + 1`, неизменяемая строка `AnalyticsInsightVersion`.
...
(сила статистики и существенность отдельно, контекст, confounders, ручное закрытие с причиной). Карточки `scope = data`
(FIX_01) помечены как качество измерения: подпись блока гипотезы «Качество измерения — не поведение клиентов» и чип
«качество данных, не поведение»; ограничение `NOT_MEASURED_STEPS` — «часть шагов не измеряется». Пустое состояние —
