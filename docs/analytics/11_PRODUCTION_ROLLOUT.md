...
(00:17…12:17 16.09) SUCCESS, Stage 06/09/10 diff 0. Отчёт — § 35. Owner smoke § 27 ПРОЙДЕН владельцем 16.09.2026
(~14:42 MSK, «OWNER SMOKE STAGE 11 ПРОЙДЕН») — § 35.18. Production после enable не менялся; Acceptance Gate § 32 закрыт
со стороны исполнителя; DONE ставит Reviewer.
```
...
Stage 06/09/10 без регрессий (diff 0). Owner smoke § 27 ПРОЙДЕН владельцем 16.09 (~14:42 MSK) — § 35.18. Production после
enable 23:17 15.09 не менялся. Все пункты Acceptance Gate (§ 32) закрыты со стороны исполнителя; DONE ставит Reviewer.
...
```text
§ 27 — ПРОЙДЕН: подтверждение владельца 16.09.2026 (~14:42 MSK), дословно: «OWNER SMOKE STAGE 11 ПРОЙДЕН».
Проверялось владельцем под своим ADMIN: https://raspechatkaa.ru/crm/analytics → «Рост / Изменения» (чек-лист § 27).
Подтверждение текстовое, без детализации по пунктам и без скриншотов (по § 27 допустимо); исполнитель под учётной
записью владельца не входил, production после enable 15.09 23:17 не менял.
Ориентиры на момент проверки (после тика 00:17 16.09, cutoff 15.09): A — «Окна несопоставимы», окна 09–11.09 → 13–15.09
(3 дня), 2,56 % (2 из 78) → 8,11 % (3 из 37), оговорки: дни недели, смесь источников, смена определения, инцидент B;
B — «Завершено», «Оценок ещё нет» (полного дня внутри инцидента нет). Числа при следующих тиках сдвигаются с новым днём.
Чек-лист § 27 (пункты закрыты общим подтверждением владельца):
  [x] плашка NO_VARIANT_ASSIGNMENT и дисклеймер
  [x] A — «Окна несопоставимы», не «+…%»; B — завершённое изменение-граница
  [x] A: окна с исключённым 12.09, ФАКТ / ИНТЕРПРЕТАЦИЯ / ЧТО ДЕЛАТЬ, оговорки с инцидентом, версии, «Оценить заново»
  [x] форма регистрации (метрики другой области — «только контекст», московское время)
  [~] телефон 390 px — отдельно владельцем не отмечено (подтверждено production-JSON в сборке кандидата:
      screenshots/11_growth/prod-growth-mobile-390.png, scrollWidth 390)
```
...
3. Owner smoke § 27 — ПРОЙДЕН владельцем 16.09 (§ 35.18).
