...
/**
 * После этого часа дайджест за сегодня уже не уходит.
 *
 * Проверка ежечасная и умеет догонять пропущенное, иначе перезагрузка сервера
 * в 10:00 съедала бы весь день. Но без верхней границы это же свойство даёт
 * ночной пинг: поставил задачу в 23:00 — бот тут же будит весь чат. Окно
 * закрывает и второй случай: если сервер лежал с утра до ночи, лучше
 * промолчать до завтра, чем звонить людям в час ночи.
 */
export const DIGEST_LAST_HOUR = 21;

/** Пора ли слать дайджест: рабочее окно 10:00–21:59 по Москве. */
export function isWithinDigestWindow(date: Date): boolean {
  const hour = moscowHour(date);
  return hour >= DIGEST_HOUR && hour <= DIGEST_LAST_HOUR;
}
