    ...
    for (const f of funnels) {
      ...
      detected.push(
        build(d, ctx, {
          ...
          recommendation: rec(
            `Проверить измерение шага(ов) ${labels(relevant)} в счётчике Метрики: если событие уже отправляется сайтом — завести на него цель; если события нет — зафиксировать шаг как намеренно неизмеряемый или запланировать измерение отдельным решением. Событийную модель сайта (web-photo) в рамках этапа 12 не менять; до появления измерения выводы об отвале на этих шагах не делать.`,
            'IMPROVE_DATA_QUALITY',
          ),
          evidence: {
