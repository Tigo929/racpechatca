...
function emptyBucket(): PnlRaw {
  return {
    orderCount: 0, photoCount: 0, tshirtCount: 0,
    totalRevenue: 0, photoRevenue: 0, tshirtRevenue: 0, deliveryCost: 0,
    materialsPhoto: 0, materialsTshirt: 0,
    deliverySupplies: 0, equipment: 0, marketing: 0, other: 0,
    salaryPaid: 0,
  };
}
...
function addExpense(b: PnlRaw, e: ExpenseRow): void {
  switch (e.category) {
    case 'MATERIALS_PHOTO':   b.materialsPhoto   += e.amount; break;
    case 'MATERIALS_TSHIRT':  b.materialsTshirt  += e.amount; break;
    case 'DELIVERY_SUPPLIES': b.deliverySupplies += e.amount; break;
    case 'EQUIPMENT':         b.equipment        += e.amount; break;
    case 'MARKETING':         b.marketing        += e.amount; break;
    default:                  b.other            += e.amount; break;
  }
}

function sumBuckets(buckets: PnlRaw[]): PnlRaw {
  return buckets.reduce((acc, b) => {
    (Object.keys(acc) as (keyof PnlRaw)[]).forEach((k) => { acc[k] += b[k]; });
    return acc;
  }, emptyBucket());
}
...
function finalize(b: PnlRaw) {
  ...
  const operatingExpenses = b.deliverySupplies + b.equipment + b.marketing + b.other;
  ...
  const margin = b.totalRevenue > 0
    ? Math.round((netProfit / b.totalRevenue) * 1000) / 10
    : 0;
  const avgCheck = b.orderCount > 0 ? Math.round(b.totalRevenue / b.orderCount) : 0;
  return {
    ...b,
    netRevenue,
    cogs,
...
export class ReportsService {
  ...
  async getMonthlyReport(year: number) {
    ...
    const { orders, expenses, salaryPayments } = await this.fetchPeriod(start, endExclusive);
    ...
    for (const p of salaryPayments) buckets[p.createdAt.getMonth()].salaryPaid += p.amount;
  ...
  async getWeeklyReport(year: number, month: number) {
    ...
    const findWeek = (d: Date) => weekDefs.findIndex((w) => d >= w.start && d < w.endExclusive);
  ...
  private buildWeeks(year: number, month: number) {
    ...
    const result: {
      weekNum: number; start: Date; endExclusive: Date;
