...
export function needsShipmentStatus(order: {
  deliveryMethod: string;
}): boolean {
...
export function fulfillmentError(
  ...
  const external =
    order.productCategory === 'CANVAS' || order.productCategory === 'TSHIRT';
  ...
  if (next === 'SHIPMENT_CREATED') {
    if (!shipment)
      return 'Для самовывоза отгрузка не нужна. Используйте статус «Готов к выдаче».';
    if (!ready && !(order.status === 'SENT' && !external))
      return 'Сначала переведите заказ в «Готов», затем создавайте отгрузку.';
  }
  if (next === 'SENT' && !external) {
    if (shipment && order.status !== 'SHIPMENT_CREATED')
      return 'Сначала создайте отгрузку, затем переводите заказ в «Отправлен».';
    if (!shipment && !ready)
      return 'Сначала подготовьте заказ к выдаче, затем отметьте «Выдан клиенту».';
  }
  if (next === 'PAID') {
    if (!external && order.status !== 'SENT')
      return shipment
    ...
    if (external && (shipment ? order.status !== 'SHIPMENT_CREATED' : !ready))
      return shipment
