...
export class DtoCreateLead {
  ...
  /**
   * Что делать с краями фотографии при натяжке.
   *
   * GALLERY — изображение продолжается на торцах, края кадра уходят на
   * боковые стороны. WHITE — кадр остаётся на лицевой стороне целиком,
   * торцы белые.
   *
   * Печатнику это меняет подготовку файла, а клиенту — что он увидит на
   * стене, поэтому выбор обязан доехать до примечания. Доплаты нет:
   * производство одно и то же, разница только в раскладке файла.
   */
  @IsOptional()
  @IsIn(['GALLERY', 'WHITE'])
  canvasEdge?: 'GALLERY' | 'WHITE';
  ...
  // Яндекс.Директу. UTM закрывают остальные каналы. С 11.09.2026 ложатся
  // в колонки OrderPhoto (utmSource … utmTerm), а не только в примечание.
