 ...
 *
 * Координаты нормализованы: x, y и размер — доли зоны печати, а не пиксели.
 * Благодаря этому согласование открывается через неделю на другом экране
 * (и даже после замены фотографии мокапа) ровно там, где его оставили.
 *
 * Экранный размер и физический разведены намеренно: макет раскладывают
 * глазом, а миллиметры называют производству. Подробнее — у viewWidth.
 */
export interface ApprovalSideState {
  ...
  heightMm: number;
  /**
   * Экранный размер принта — доля зоны печати, а не миллиметры.
   *
   * Раньше размер на макете считался из widthMm: ввёл «28 см» — картинка
   * прыгнула, потянул за угол — поехали сантиметры. Владелец раскладывает
   * макет глазом, а миллиметры называет производству, и связывать эти два
   * действия оказалось вредно: одно всё время портило другое.
   *
   * Теперь картинку тянут мышкой, а widthMm/heightMm остаются записью для
   * отчёта и исполнителя. Доля, а не пиксели, — чтобы раскладка пережила
   * замену фотографии мокапа, как это уже сделано для x/y.
   *
   * 0 — у согласований, сохранённых до этой правки: там размер по-прежнему
   * берётся из миллиметров, и старые макеты выглядят как раньше.
   */
  viewWidth: number;
  viewHeight: number;
