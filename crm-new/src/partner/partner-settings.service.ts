...
import type { CanvasPriceMode } from 'src/canvas/canvas-production-price';
import { OrderSettlement, settleOrder } from './partner-settlement';
import {
...
export interface UpdatePartnerSettingsDto {
  ...
  /** Какой прайс производства на холст действует сейчас. */
  canvasPriceMode?: CanvasPriceMode;
}
...
export class PartnerSettingsService {
  ...
  async update(dto: UpdatePartnerSettingsDto) {
    ...
    return this.prisma.partnerSettings.update({
      ...
      data: {
        ...
        deliveryPriceYandexPvz: dto.deliveryPriceYandexPvz,
        canvasPriceMode: dto.canvasPriceMode,
      },
    });
  }
  ...
  private async fallbackAuthor(tx: Prisma.TransactionClient): Promise<string> {
    ...
    if (!admin)
      throw new Error('Нет ни одного администратора для авторства расхода');
    return admin.id;
  }
}
